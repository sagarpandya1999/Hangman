...
def multiplayer():
    ...
    while (t < 2 * NoofRound):
        if (Turn == 1):
            print(Player1 + " turn")
            MovieName = input("give word(you can enter space also)")
...
def singleplayer():
    print("this is multiplayer and at developing stage")
    print("we will update you soon")
    lines  = []
    with open('Hangman3.txt') as f:
        lines = f.read().splitlines()
        print(lines)


#Driver code
print("welcome to the game named Hangman")
print("INSTRUCTIONS:")
print("       this is two player game")
print("       you have to word and some char. for hint to other player")
print("       other player have to enter one by one char. that he wants to put in string")
...
print("BEST OF LUCK :)")
print(" ")
playmode = input("press 1 if you want to play multiplayer and 2 for single player")
if(playmode == '1'):
    multiplayer()
elif(playmode == '2'):
    singleplayer()
    ...
    while(1):
        playmode = input("please enter valid choice")
        if(playmode == '1'):
            multiplayer()
            break
        elif(playmode == '2'):
